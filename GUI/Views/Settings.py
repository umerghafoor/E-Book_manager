...
class SettingsView(QWidget):
    def __init__(self, parent=None):
        ...
        self.load_recursive_folder_button = QPushButton("Load Folder (Recursive)")
        
        self.load_book_button.clicked.connect(self.load_book)
        self.load_book_folder_button.clicked.connect(self.load_folder)
        self.load_recursive_folder_button.clicked.connect(self.load_recursive_folder)
        ...
        main_layout.addWidget(self.load_recursive_folder_button, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        if folder_path:
            ...
            for filename in os.listdir(folder_path):
                if filename.lower().endswith('.pdf'):
                    ...
                    details_dialog = BookDetailsDialog(file_path)
                    if details_dialog.exec() == QDialog.DialogCode.Accepted:
                        book_details = details_dialog.get_details()
                        BookDatabase().create_database()
                        BookDatabase().add_book(
    ...
    def load_recursive_folder(self):
        # Open dialog to select a folder
        folder_path = QFileDialog.getExistingDirectory(self, "Select Folder")
